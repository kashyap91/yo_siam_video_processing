...
def get_frames(video_name):
    ...
    if video_name.endswith('avi') or \
        ...
        vid_name = "./videos5/{}".format(video_name)
...
def main():
    ...
    #yolo Initialize
    device = torch_utils.select_device(False)
            ...
            for *xyxy,conf,cls_conf,cls in det:
                if cls == 0:
                    if conf >= 0.75:
                        ...
                        with open('./videos5/'+video_name_yolo+'_yolo.csv', 'a', newline='') as csvfile:
    ...
    #video_list = []
    video_list_mp4 = glob1("./videos5", "*.mp4")
    #video_list.append(video_list_mp4)
    video_list_avi = glob1("./videos5","*.avi")
    ...
    for video_name in itertools.chain(video_list_mp4, video_list_avi):
        ...
        #cords = [1,2,3,4]
        try:
            df = pd.read_csv('./videos5/'+video_name+'_yolo.csv', delimiter=',', header=None)
            cords = [list(x) for x in df.values]
        except:
            continue
                ...
                for frame in get_frames(video_name):#(args.video_name):
                    if first_frame:
                        ...
                        #cv2.waitKey(40)
                            ...
                            with open('./videos5/vid-'+str(video_name)+'-tracking-'+str(object_counter)+'-object-'+str(cord)+'.csv', 'a', newline='') as csvfile:
